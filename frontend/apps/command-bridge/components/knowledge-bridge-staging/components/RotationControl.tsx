...
interface RotationControlProps {
  ...
  onRotate: (newRotation: { x: number; y: number }) => void;
}

export const RotationControl: React.FC<RotationControlProps> = ({ rotation, onRotate }) => {
    ...
    <div className="absolute bottom-52 left-1/2 -translate-x-1/2 z-40 flex flex-col items-center pointer-events-auto group touch-none">
      <div 
        className={`
           ...
           ${isDragging ? 'border-cyan-500/50 shadow-[0_0_20px_rgba(34,211,238,0.2)]' : 'hover:bg-slate-800/50'}
        `}
      ...
      <div className="w-[1px] h-8 bg-gradient-to-b from-slate-700/50 to-transparent"></div>
    </div>
  );
};
