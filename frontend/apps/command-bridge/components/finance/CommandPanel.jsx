...
import { Search, Monitor, Cpu, TrendingUp, Zap, Shield, AlertTriangle, Brain, Radar } from 'lucide-react';
import { CRYPTO_SYMBOLS, STOCK_SYMBOLS, FUTURES_SYMBOLS, STRATEGY_PRESETS } from './constants';
import OpportunityScannerModal from './OpportunityScannerModal.jsx';
...
export const CommandPanel = ({
  ...
  return (
    <div className="flex flex-col h-full bg-soma-900 border-r border-soma-800">
      ...
      <div className="p-3 border-b border-soma-800/50 shrink-0">
            ...
            <div className=\"flex gap-2\">
                <button 
                    ...
                    className=\"w-full py-1.5 bg-gradient-to-r from-blue-500/20 to-transparent border border-blue-500/50 rounded flex items-center justify-center gap-2 text-blue-400 font-bold text-[10px] hover:bg-blue-500/10 transition-all group\"
                >
                    <Radar className=\"w-3 h-3 group-hover:animate-pulse\" />
                    SCANNER
                </button>
                <button 
                    onClick={onAnalyze}
      ...
      <OpportunityScannerModal 
        isOpen={isScannerOpen} 
        onClose={() => setIsScannerOpen(false)} 
        assetType={assetType}
        onSelect={(opportunity) => {
            onSymbolSelect(opportunity.symbol);
            const preset = STRATEGY_PRESETS.find(p => p.id === opportunity.strategy);
