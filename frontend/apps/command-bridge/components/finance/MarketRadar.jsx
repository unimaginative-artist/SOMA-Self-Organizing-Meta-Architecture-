...
export const MarketRadar = ({ tickers, onSelect, selectedSymbol, riskMetrics }) => {
    ...
    return (
        <div className="h-full bg-soma-950 flex flex-col lg:flex-row overflow-hidden relative border-t border-soma-800">
                    ...
                    {tickers.map(ticker => {
                        ...
                        return (
                            <div
                                ...
                                className={`absolute w-2 h-2 rounded-full cursor-pointer transition-all duration-500 hover:scale-150 z-20 ${ticker.momentum > 0
            ...
            <div className="absolute bottom-0 left-0 w-full h-8 bg-soma-950/90 backdrop-blur border-t border-soma-800 flex items-center z-20">

                {/* 1. PINNED SECTION (Selected + Top Mover) */}
                <div className="flex shrink-0 z-30 h-full bg-soma-950 border-r border-soma-800 shadow-[20px_0_20px_rgba(0,0,0,0.8)]">
                    {/* Selected Symbol */}
                    {tickers.filter(t => t.symbol === selectedSymbol).map(ticker => (
                        <button
                            key={ticker.symbol}
                            onClick={() => onSelect(ticker.symbol)}
                            className="px-4 h-full flex items-center gap-2 text-[10px] font-mono whitespace-nowrap bg-soma-800 text-white border-r border-soma-800/50"
                        >
                            <span className="font-bold text-soma-accent">PINNED</span>
                ...
                {/* 2. SCROLLING TICKER AREA */}
                <div className="flex-1 relative overflow-hidden h-full group mask-linear-fade">
                    <div className="flex h-full animate-marquee w-max">
                        ...
                        {[...tickers, ...tickers].map((ticker, i) => (
                            <button
                                key={`${ticker.symbol}-${i}`}
                                onClick={() => onSelect(ticker.symbol)}
                                className={`px-4 h-full flex items-center gap-2 text-[10px] font-mono whitespace-nowrap border-r border-soma-800 hover:bg-soma-800/50 transition-colors ${selectedSymbol === ticker.symbol ? 'bg-soma-800/30 text-white' : 'text-slate-500'}`}
                            >
                                <span className="font-bold">{ticker.symbol}</span>
                                <span className={ticker.change >= 0 ? 'text-soma-success' : 'text-soma-danger'}>
                                    {ticker.price.toFixed(2)}
                                </span>
                                <span className={`text-[9px] opacity-70 ${ticker.change >= 0 ? 'text-soma-success' : 'text-soma-danger'}`}>
